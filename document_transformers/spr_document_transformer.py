...
class SPRTextTransformer(BaseDocumentTransformer):
   """
   ...
   def from_documents(self, docs: List[Document]) -> List[str]:
       ...
       for doc in docs:
           try:
               messages = self.get_prompt_messages(doc.page_content)
               response = self.llm(messages)
               responses.append(Document(page_content=response))
   ...
   def get_prompt_messages(self, text: str) -> List[str]:
       """
